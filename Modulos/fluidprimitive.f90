...
! open(unit=5, file='TOVstar2_Model5_SinPolvo/data4eosOnly.txt', status='replace')
...
!   Case 2. Dust Part. p=0.
     else if (fluid_p(l,i)<=1.5d0*patmos) then

        fluid_p(l,i) = patmos
        fluid_q(l,i) = 0.d0

        fluid_v(l,i) = fluid_cS(l,i)/(fluid_cE(l,i) + fluid_cD(l,i) + patmos + fluid_q(l,i)) &
                   /(A(l,i)*exp(4.d0*phi(l,i)))
        if (fluid_v(l,i) /= fluid_v(l,i)) then
          print * 
          print *, "Error. There is a nan in the fluid speed.(Dust)"
          print *, "v: ",fluid_v(l,i)," at i=",i
          call die
        end if
...
!       Find specific internal energy (it should be positive).
        aux = fluid_cD(l,i)*(W - 1.d0) + (patmos + fluid_q(l,i))*(W**2 - 1.d0)
...
!       Find enthalpy.
        fluid_h(l,i) = 1.d0 + fluid_e(l,i) + (patmos + fluid_q(l,i))/fluid_rho(l,i)
...
!       Find speed of sound.
        ...
        ! Dust part.
        else if (fluid_EOS=="TOV2_ev" .or. fluid_EOS=="TOV2_EoS" ) then
          fluid_vs(l,i) = 1.d-6
        end if
...
!       Find value of v using old value of pressure.
        ...
        if (fluid_v(l,i) /= fluid_v(l,i)) then
          print *, "Error. There is a nan in the fluid speed.(Core)"
          print *, "v: ",fluid_v(l,i)," at i=",i
          call die
        end if
...
!       really this is not a good idea.
          ...
          if (i==1) then
            fluid_vs(l,i) = soundVelocity2(fluid_p(l,i+1),p1,fluid_rho(l,i-1),fluid_rho(l,i),fluid_h(l,i))
          else
            fluid_vs(l,i) = soundVelocity2(fluid_p(l,i-1),p1,fluid_rho(l,i-1),fluid_rho(l,i),fluid_h(l,i))
          end if
...
!       the TOV equations), then we just set the difference to 0.
        ...
        if (fluid_EOS/="none") then
           ...
           else if (fluid_EOS=="TOV2_ev") then
              f1 = Omega_TOV2(l,i)*fluid_rho(l,i) - p1
           else if (fluid_EOS=="TOV2_EoS") then
              f1 = EoS(fluid_rho(l,i)) - p1   
        ...
        if (fluid_EOS/="none") then
           ...
           if (j==1) then
              if (fluid_EOS=="ideal") then
                 ...
                 aux = Omega_TOV2(l,i)*fluid_rho(l,i)
              else if (fluid_EOS=="TOV2_EoS") then
                  aux = EoS(fluid_rho(l,i)) 
              else
                 aux = fluid_p(l,i)
              end if
           else
...
!    Characteristic speeds.
  ...
  if (filterTOV2 .or. vsSecondOrderTOV2) call filter_tov2
...
function soundVelocity2(P1,P0,rho1,rho0,h)
  use param
  real(8) P1,P0,rho1,rho0,h
  real(8) h0,vs0
  real(8) dP, drh,outVs
  real(8) soundVelocity2
  ...
  if (abs(drh)>1e-10) then
    outVs=sqrt(dP/drh)
  else 
    outVs=1.d-4
  end if

  soundVelocity2=outVs

end function soundVelocity2


function EoS(rho_input)
  use param
  use arrays
  ...
  if (rho_input<rho_minim) then
    EoS = 0.d0

  else if (rho_input>rho_minim .and. rho_input<rho_maxim) then

    do i = 1, Nr
        if ((rho_eos(l,i) <= rho_input .and. rho_eos(l,i + 1) >= rho_input) .or. &
            (rho_eos(l,i) >= rho_input .and. rho_eos(l,i + 1) <= rho_input)) then
            ! Linear interpolation: p_test = p1 + (rho_test - rho1) * (p2 - p1) / (rho2 - rho1)
            EoS = p_eos(l,i) + (rho_input - rho_eos(l,i)) * (p_eos(l,i + 1) - p_eos(l,i)) / &
                     (rho_eos(l,i + 1) - rho_eos(l,i))
            exit
        end if
    end do
...
!   x = a5_eos*rho_input**4  +  a4_eos*rho_input**3  +  a3_eos*rho_input**2  +  a2_eos*rho_input  +  a1_eos
    x = a3_eos*rho_input**2  +  a2_eos*rho_input  +  a1_eos
    EoS = x
